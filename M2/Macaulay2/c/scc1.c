...
static void usage() {
  ...
  printf("    -verbose      show files opened\n");
}
...
int main(int argc, char **argv){
     ...
     for (i=1; i<argc; i++) {
     	  if (EQUAL == strcmp(argv[i],"-help") || EQUAL == strcmp(argv[i],"--help")) {
	       usage();
	       exit(0);
	       }
	  ...
	  if (EQUAL == strcmp(argv[i],"-verbose")) {
	       verbose = TRUE;
	       continue;
	       }
	  ...
	  if ( EQUAL == strcmp(".d",tail(argv[i])) || EQUAL == strcmp(".dd",tail(argv[i])) ) {
		    ...
		    }
	       {
		    ...
		    if (NULL == freopen(n,"w", stdout)) fatal("can't open file %s",n);
		    if (verbose) fprintf(stderr,"-- writing %s\n",n);
	       ...
	       if (TRUE) {
		    if (NULL == freopen(outfilename,"w", stdout)) fatal("can't open file %s",outfilename);
		    if (verbose) fprintf(stderr,"-- writing %s\n",outfilename);
